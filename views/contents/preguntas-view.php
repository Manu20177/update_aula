...
if (isset($_GET['views'])) {
    ...
    if ($partes[0] === 'preguntas' && !empty($partes[1]) && is_numeric($partes[1])) {
        $id_clase = intval($partes[1]);
    } else {
        die("Ruta no reconocida o ID inválido");
    }
} else {
...
$yaRespondio = $preguntaController->check_evaluacion_respondida_controller($_SESSION['userKey'], $id_clase);

// Cargar preguntas de la clase
$preguntas = $preguntaController->cargar_preguntas_controller($id_clase);
?>
    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-xs-12">
                ...
                <?php if ($yaRespondio): 
                    $nota=$preguntaController->obtener_nota($_SESSION['userKey'], $id_clase);;
                ...
                ?>
                    ...
                    <div class="alert alert-success text-center" style="margin-top: 25%;" role="alert">
                        Ya has completado esta evaluación, tu nota es <?php echo $nota ?>. ¡Gracias!
                    </div>
                <?php else: ?>
                    <form action="<?php echo SERVERURL; ?>ajax/ajaxEvaluacion.php" method="POST" class="ajaxDataForm" data-form="evaluacion">
                        <h3 class="text-titles text-center"><i class="zmdi zmdi-edit"></i> Evaluación</h3>
                       ...
                       <p class="text-center">
                            <button type="submit" id="enviarBtn" class="btn btn-success btn-raised" disabled>Enviar evaluación</button>
                        </p>
