...
$Datosc=$insVideo->execute_single_query("
      ...
      DATE_FORMAT(ca.fecha_termino, '%d / %m / %Y') as fecha,tc.descripcion as Norma 
    ...
    ");
$horas=$insVideo->execute_single_query("
          SELECT SUM(horas) as Total_horas FROM `curso_clase` WHERE `id_curso`='$code';
    ");
if ($horas && $fila = $horas->fetch()) {
...
try {
    ...
    $pdf->Cell($ancho, $alto, utf8_decode($fecha), 0, 0, 'C');
     ...
     // Horas - Courier, cursiva, tamaño 12
    $pdf->SetFont('Times', 'BI', 16);
    $pdf->SetTextColor(0, 0, 0); // Negro
    // Posición X,Y donde comienza el "contenedor"
    $x = 149;
    ...
    // Escribir texto centrado usando Cell()
    $pdf->SetXY($x, $y);
    $pdf->Cell($ancho, $alto, utf8_decode($total_horas), 0, 0, 'C');

    // NORMA - Times, cursiva, tamaño 19
    ...
    $ancho = 250; 
    $alturaLinea = 7; // Altura por línea

    // Texto dinámico
    $texto = "Norma de " . $norma;

    // Mover a posición inicial
    $pdf->SetXY($x, $y);

    // Escribir con MultiCell()
    $pdf->MultiCell($ancho, $alturaLinea, utf8_decode($texto), 0, 'C');
