...
// Cargar preguntas
$preguntas = $encuestaController->cargar_preguntas_evaluacion_controller($id_clase);
...
<form action="<?php echo SERVERURL; ?>ajax/ajaxEncuesta.php" method="POST" class="ajaxDataForm" data-form="evaluacion">
    <h3 class="text-titles text-center"><i class="zmdi zmdi-equalizer"></i> Evaluacion <?php echo $clase_title?></h3>
    
    <!-- Botón para eliminar toda la encuesta -->
    <button type="button" class="btn btn-danger btn-sm delete-btn-all" 
			data-idclase="<?= $id_clase ?>">
		<i class="zmdi zmdi-delete"></i> Eliminar toda la Evaluacion
	</button>
           ...
           <button type="button" class="btn btn-sm btn-warning delete-btn-pregunta" 
            ...
            <?php if ($p['tipo'] == 'multiple'): ?>
                ...
                <div class="form-check">
                  ...
                  <div class="form-check">
                    <label class="form-check-label" style="color: green;">La respuesta correcta es: <?= htmlspecialchars($p['respuesta']) ?></label>
                </div>
