		...
		public function pagination_comment_controller($codeC,$Pagina,$Registros,$id){
			...
			if($Total>=1){
				...
				foreach($Datos as $rows){
					$query1=self::execute_single_query("SELECT c.Usuario,c.Genero,c.Tipo,CONCAT(e.Nombres,' ',e.Apellidos) as Nombres FROM cuenta c LEFT JOIN estudiante e on e.Codigo=c.Codigo WHERE c.Codigo='".$rows['Codigo']."'");
					$data1=$query1->fetch();
