<?php
    ...
    class encuestaModel extends mainModel {
        ...
        public function get_preguntas_by_clase_model($id_clase) {
            $query = self::connect()->prepare("SELECT e.*, c.Titulo FROM encuesta_rapida e LEFT JOIN clase c on c.id=e.id_clase WHERE id_clase = :id_clase");
            $query->bindParam(":id_clase", $id_clase);
            $query->execute();
            return $query;
        }
        /*----------  Obtener preguntas por clase  ----------*/
        public function get_preguntas_by_evaluacion_model($id_clase) {
            $query = self::connect()->prepare("SELECT p.*, c.Titulo FROM preguntas_rapidas p LEFT JOIN clase c on c.id=p.id_clase WHERE p.id_clase = :id_clase");
            $query->bindParam(":id_clase", $id_clase);
            $query->execute();
            return $query;
        }
        ...
        public function save_respuesta_model($data) {
            $query = self::connect()->prepare("
                INSERT INTO encuesta_respuesta (id_usuario, id_pregunta, respuesta_usuario)
